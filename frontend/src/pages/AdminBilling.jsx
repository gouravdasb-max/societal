...
export default function AdminBilling() {
  ...
  return (
    <AppLayout>
      ...
      {showSettings && (
        <form className="card" style={{ marginBottom: 24, animation: "float-in .35s ease both", borderLeft: "4px solid var(--accent)" }} onSubmit={handleSaveSettings}>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {bills.filter((b) => (b.resident?.fullName || "").toLowerCase().includes(search.toLowerCase()) || (b.resident?.flatNumber || "").toLowerCase().includes(search.toLowerCase())).map((bill, i) => (
            <div key={bill._id} className="card" style={{ animationDelay: `${i * 0.04}s` }}>
